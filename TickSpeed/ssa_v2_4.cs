...
#pragma warning disable 612
    [HandlerName("SSA_V2_4")]
#pragma warning restore 612


    public class IncrementalSSA4 : IOneSourceHandler, IDoubleInputs, IDoubleReturns, IStreamHandler, IContextUses
    {
        ...
        private static alglib.ssamodel worker4;

        // analyzer - модель, которая делает анализ на основе базиса, построенного worker
        private static alglib.ssamodel analyzer4;
        ...
        static IncrementalSSA4()
        {
            double[,] dummy_basis = new double[,] { { 1 } };
            data_inside = 0;
            last_result = new double[0];
            alglib.ssacreate(out worker4);
            alglib.ssacreate(out analyzer4);
            int current_window = 1;
            int current_k = 1;
            alglib.ssasetwindow(worker4, current_window);
            alglib.ssasetwindow(analyzer4, current_window);
            alglib.ssasetalgotopkrealtime(worker4, current_k);
            alglib.ssasetpoweruplength(worker4, 5);
            //alglib.ssasetalgotopkdirect(worker, current_k);
            alglib.ssasetalgoprecomputed(analyzer4, dummy_basis, current_window, current_k);
        }
        ...
        public IList<double> Execute(IList<double> myDoubles)
        {
            ...
            alglib.ssasetwindow(worker4, window_size);
            alglib.ssasetalgotopkrealtime(worker4, k);
            if (Reset)
            {
                data_inside = 0;
            }
            if (data_inside > 0)
            {
                ...
                {
                    alglib.ssaappendpointandupdate(worker4, myDoubles[i], i == count - 1 ? update_freq : 0.0);
                    alglib.ssaappendpointandupdate(analyzer4, myDoubles[i], 0.0);
                }
            }
            else
            {
                ...
                for (int i = 0; i < count; i++)
                    vals[i] = myDoubles[i];
                alglib.ssacleardata(worker4);
                alglib.ssacleardata(analyzer4);
                alglib.ssaaddsequence(worker4, vals, count);
                alglib.ssaaddsequence(analyzer4, vals, count);
                last_result = new double[0];
                need_full_analysis = true;
            }
            data_inside = count;
            alglib.ssagetbasis(worker4, out new_basis, out sv, out dummy0, out dummy1);
            alglib.ssasetalgoprecomputed(analyzer4, new_basis, window_size, k);
            ...
            alglib.ssaanalyzelast(analyzer4, alen, out last_trend, out last_noise);
            ...
            {
                double[] fc;
                alglib.ssaforecastlast(analyzer4, Numfor, out fc);
