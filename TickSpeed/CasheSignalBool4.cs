...
#pragma warning disable 612
    [HandlerName("CasheBool4")]
#pragma warning restore 612
    public class CasheSignalBool4Class : ITwoSourcesHandler, ISecurityInput0, IBooleanInput1, IStreamHandler, IBooleanReturns
    {
        
        [HandlerParameter(Name = "Reset", Default = "true", NotOptimized = false)]
        public bool Reset { get; set; }
        private static IList<double> Tradecashe4 { get; set; }
        private static IList<bool> Boolcashe4 { get; set; }
        ...
        public IList<bool> Execute(ISecurity sec, IList<bool> bools)
        {
            ...
            if (Tradecashe4.IsNull() || Boolcashe4.IsNull() || Reset)
            {
                
                Tradecashe4 = tradeno.ToList();
                Boolcashe4 = bools.ToList();
                //Tcashe = time.ToList();
            }
            else
            {
                var s = Tradecashe4.Last();
                var delta =count - Array.FindIndex(tradeno, 0, w => w.Equals(s)) - 1;

                
                var bl = bools.Skip(count - delta).Take(delta).ToList();
                Boolcashe4.AddRange(bl);
                var tr = tradeno.Skip(count - delta).Take(delta).ToList();
                Tradecashe4.AddRange(tr);
                //var ti = time.Skip(count - delta).Take(delta).ToList();
                //Tcashe.AddRange(ti);
                
            }

            return Boolcashe4.TakeLast(count).ToArray();          
