...
namespace TickSpeed
{
    ...
    [HandlerName("VTOMULTSPEED")]
    public class VolTickOscMultSpeed : IBar2DoubleHandler
    {
        ...
        public IList<double> Execute(ISecurity security)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                values[i] = ((valueTickBuy  - valueTickSell) / (valueTickBuy + valueTickSell)) *
                            Math.Abs((valueVolBuy - valueVolSell) / (valueVolBuy + valueVolSell)) * Math.Abs(valueTickBuy - valueTickSell);
            }
            return values;
        }
    }
